...
def build_index(items):
    latest=next((x for x in items if x.kind=='daily'), None)
...
</style></head><body><div class="app"><aside><h1>Daily English<br>Lessons</h1><p class="sub">A readable archive for daily English lessons plus weekly/monthly reviews.</p><span class="badge">Last site build: {html.escape(generated)}</span><h2>Today / Latest</h2>{nav([latest] if latest else [], 'daily') if latest else '<p class="empty">No latest lesson yet.</p>'}<h2>Past daily lessons</h2>{nav(items,'daily')}<h2>Weekly reviews</h2>{nav(items,'weekly')}<h2>Monthly reviews</h2>{nav(items,'monthly')}</aside><main><div class="viewer-shell"><div class="viewer-top"><div id="current-title">{html.escape(latest_title)}</div><a id="open-link" class="open" href="{html.escape(latest_url)}" target="_blank" rel="noopener">Open full page</a></div><iframe id="viewer" src="{html.escape(latest_url)}" title="English lesson viewer"></iframe></div></main></div><script>
