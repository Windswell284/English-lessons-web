...
def standalone_nav_html(items: list[Item], current: Item) -> str:
    ...
    return f'''<script id="standalone-embedded-detector">(()=>{{let inFrame=false;try{{inFrame=window.self!==window.top;}}catch(e){{inFrame=true;}}if(inFrame&&new URLSearchParams(location.search).has('embedded'))document.documentElement.classList.add('standalone-embedded');}})();</script>
